...
window.NotionAI.Chat.Renderer = {
    ...
    createStarIcon(modelDisplayName) {
        ...
        starContainer.innerHTML = `
            <div class="star-tooltip absolute bottom-full mb-2 left-1/2 -translate-x-1/2 whitespace-nowrap"></div>
        ...
        `;
        const tooltip = starContainer.querySelector('.star-tooltip');
        if (tooltip) {
            tooltip.textContent = modelDisplayName;
        }
    ...
    appendAssistantContent(bubble, content, isFinished, modelDisplayName) {
        ...
        thinkingContainer.innerHTML = `
            <button class="thinking-toggle" type="button"></button>
        ...
        if (thinkingToggle) {
            thinkingToggle.textContent = `${modelDisplayName}'s thinking: ▸`;
        }

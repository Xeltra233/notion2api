...
- Account-card `刷新探测` and `工作区探测` actions also dump their full response JSON into the shared textarea. Even for dry-run probes, mask top-level `account_id` and recursively redact nested request-template identifiers like `user_id`, `space_id`, and transaction `actor_id` before returning to the browser.

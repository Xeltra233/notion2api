...
- 模型列表：`GET /v1/models`
- 后台控制台：直接打开根路径 `/`，默认先看到后台登录页
- Chat 模块：登录后台后进入；如果启用了 Chat password，则还需要额外完成一次聊天访问登录
...
- `API_KEY` 可以留空用于本地开放模式，也可以设置为自定义值启用 Bearer 保护；该保护同时覆盖 `/v1/...` 与 `/v1beta/...` 兼容入口

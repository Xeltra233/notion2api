...
with TestClient(app) as client:
    ...
    refresh_status = client.get("/v1/admin/session/refresh-status", headers=headers)
    ...
    refresh_diagnostics = client.get(
        "/v1/admin/session/refresh-diagnostics", headers=headers
    )
    workspace_diagnostics = client.get(
        "/v1/admin/workspaces/diagnostics", headers=headers
    )
        ...
        json.dumps(
            {
                ...
                "auto_status_contains_secrets": auto_status.json().get(
                    "contains_secrets"
                ),
                "refresh_status_mode": refresh_status.json().get("response_mode"),
                "refresh_status_contains_secrets": refresh_status.json().get(
                    "contains_secrets"
                ),
                "workspace_status_mode": workspace_status.json().get("response_mode"),
                "workspace_status_contains_secrets": workspace_status.json().get(
                    "contains_secrets"
                ),
                "workspace_rows_mode": workspace_rows.json().get("response_mode"),
                "workspace_rows_contains_secrets": workspace_rows.json().get(
                    "contains_secrets"
                ),
                "refresh_diagnostics_mode": refresh_diagnostics.json().get(
                    "response_mode"
                ),
                "refresh_diagnostics_contains_secrets": refresh_diagnostics.json().get(
                    "contains_secrets"
                ),
                "workspace_diagnostics_mode": workspace_diagnostics.json().get(
                    "response_mode"
                ),
                "workspace_diagnostics_contains_secrets": workspace_diagnostics.json().get(
                    "contains_secrets"
                ),
            },
            ensure_ascii=False,

    ...
    payload = ChatCompletionResponse(
        ...
        choices=[
            ChatMessageResponseChoice(
                index=0,
                message=ChatMessage(role="assistant", content="compat openai reply https://example.com/search-result"),
                finish_reason="stop",
            )
        ],
    ...
    try:
        ...
        chat_result = run_async(
            chat_module.create_chat_completion(
                ...
                req_body=chat_module.ChatCompletionRequest(
                    model="gpt-5.4",
                    messages=[chat_module.ChatMessage(role="user", content="hello from openai compat")],
                    temperature=0.2,
                    top_p=0.9,
                    max_tokens=64,
        ...
        chat_stream_result = run_async(
            chat_module.create_chat_completion(
                ...
                req_body=chat_module.ChatCompletionRequest(
                    model="gpt-5.4",
                    messages=[chat_module.ChatMessage(role="user", content="stream please")],
                    stream=True,
                ),
                background_tasks=BackgroundTasks(),
        ...
        search_result = run_async(
            chat_module.create_chat_completion(
                ...
                req_body=chat_module.ChatCompletionRequest(
                    ...
                    messages=[chat_module.ChatMessage(role="user", content="search variant please")],
                ),
                background_tasks=BackgroundTasks(),
        ...
        try:
            ...
            invalid_req.messages = chat_module._normalize_request_messages(invalid_req.messages)
        ...
        assert chat_payload["choices"][0]["message"]["content"] == "compat openai reply https://example.com/search-result", chat_payload
        assert chat_payload["usage"] == {"prompt_tokens": 17, "completion_tokens": 8, "total_tokens": 25}, chat_payload
        assert chat_response_obj.headers.get("X-Conversation-Id") == "compat-openai-conv", dict(chat_response_obj.headers)

        assert isinstance(chat_stream_result, StreamingResponse), type(chat_stream_result)
        ...
        chat_stream_payloads = [json.loads(frame[5:].strip()) for frame in chat_stream_frames[:-1]]
        assert chat_stream_payloads[0]["choices"][0]["delta"]["role"] == "assistant", chat_stream_payloads
        stream_text = "".join(
        ...
        assert chat_stream_payloads[-1]["choices"][0]["finish_reason"] == "stop", chat_stream_payloads[-1]
        ...
        assert responses_payload["output_text"] == "compat openai reply https://example.com/search-result", responses_payload
        assert responses_payload["output"][0]["role"] == "assistant", responses_payload
        assert responses_payload["usage"] == {"prompt_tokens": 17, "completion_tokens": 8, "total_tokens": 25}, responses_payload

        assert isinstance(responses_stream_result, StreamingResponse), type(responses_stream_result)
        responses_stream_text_raw = run_async(collect_streaming_body(responses_stream_result))
        ...
        assert str(responses_stream_result.media_type or "").startswith("text/event-stream")
        assert responses_event_names[:2] == ["response.created", "response.output_item.added"], responses_event_names
        assert "response.output_text.delta" in responses_event_names, responses_event_names
        assert responses_event_names[-2:] == ["response.completed", None] or responses_event_names[-1] == "response.completed"
        ...
        completed_event = next(event for event in responses_events if event["event"] == "response.completed")
        assert completed_event["data"]["response"]["output_text"] == "compat openai stream", completed_event

        search_payload = search_result.model_dump()
        assert search_payload["choices"][0]["message"]["content"] == "compat openai reply https://example.com/search-result", search_payload
        assert search_payload["search_metadata"] == {
            "queries": ["search variant please"],
            "sources": ["https://example.com/search-result"],
        }, search_payload
        assert search_response_obj.headers.get("X-Conversation-Id") == "compat-openai-conv", dict(search_response_obj.headers)
